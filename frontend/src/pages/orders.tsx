                    ...
                    {order.orderItems?.map((oi, i) => (
                      <div key={oi.id || i} className="flex justify-between items-start text-sm">
                        <div className="space-y-0.5">
                          ...
                          {oi.notes && <p className="text-xs text-rose-455 font-medium italic">*{oi.notes}</p>}
                        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 pt-2">
            ...
            <div className="space-y-3">
              ...
              <div className="max-h-56 overflow-y-auto space-y-3 pr-1">
                {orderItems.map((item, index) => (
                  <div key={index} className="flex gap-2 items-center bg-slate-900/40 p-2.5 rounded-lg border border-border/50">
                    <select
                      value={item.menuItemId}
                      onChange={(e) => handleItemChange(index, "menuItemId", e.target.value)}
                      className="flex-1 bg-slate-950 border border-border rounded p-1.5 text-xs text-foreground focus:outline-none"
                    >
                      {menuItems.map(dish => (
                    ...
                    <input
                      ...
                      className="w-16 bg-slate-950 border border-border rounded p-1.5 text-xs text-foreground text-center focus:outline-none"
                    />
            ...
            <DialogFooter className="pt-4 border-t border-border/60">
              <button
                ...
                className="px-4 py-2 rounded-lg border border-border text-sm text-foreground hover:bg-slate-900 transition-all font-medium cursor-pointer"
              >
                Cancel
              </button>
              <button
